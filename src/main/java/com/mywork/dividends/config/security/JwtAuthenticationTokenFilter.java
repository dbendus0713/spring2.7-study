    ...
    if (SecurityContextHolder.getContext().getAuthentication() == null) {
      ...
      if (authorizationToken != null) {
        try {
          ...
          if (isNotEmpty(name) && authorities.size() > 0) {
            JwtAuthenticationToken authentication =
                new JwtAuthenticationToken(new JwtAuthentication(name),
                    null,
                    !authorities.stream().anyMatch(ga -> ga.getAuthority().equals("ROLE_USER")),
                    authorities);
